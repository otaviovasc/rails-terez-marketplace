...
# CONTROLLER DO CHECKOUT WHATSAPP
  def load_order
    @order = current_order
    @whatsapp_link = build_whatsapp(@order)
    redirect_to(cart_path) && return unless @order
  end

  def build_whatsapp(order)
    products = order.line_items
    items = String.new
    products.each do |product|
      items << "%0A*#{product.quantity}*%20#{product.name}%20R$#{product.price}%0A"
    end
    return "https://api.whatsapp.com/send?phone=5518996469432&text=*Pedido%20numero%20#{order.id}*.%0A#{items}%0APre%C3%A7o%20total:%20#{order.total}%20#{order.currency}"
  end
